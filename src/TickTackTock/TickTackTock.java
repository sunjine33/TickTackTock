...
public class TickTackTock {
	...
	private static void playGame() {
		if (count == 0) {
			...
			System.out.println("게임을 재시작합니까??? Y/n");
			String request = sc.next();
			if(request.equals("Y")) {
				whoWin = null;
				b = new Board();
	...
	private static void Play(String first_second) {
		if (whoWin.equals("user 선제공격")) {
			userFirst();
		} else if (whoWin.equals("com 선제공격")) {
			comFirst();
		}
	...
	public static void userFirst() {
		...
		while (true) {
			...
			System.out.println("User Turn 어디에 두십니까?");
			...
			System.out.println();
			System.out.println("Com turn 어디에 두십니까??");
			flag = b.comPutStone();
			if(flag == false) {
				b.win("com win");
				break;
